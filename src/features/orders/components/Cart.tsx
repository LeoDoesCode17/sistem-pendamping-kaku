  ...
  return (
    <div className="bg-maroon rounded-lg p-6 h-fit sticky top-6">
      ...
      <div className="space-y-4 mb-6">
        {items.length === 0 ? (
          <div className="bg-white text-gray-500 rounded-lg border-2 text-center py-12">
            <svg
          ...
          items.map((item) => (
            <div
              key={item.id}
              className="bg-white text-maroon rounded-lg p-4"
            >
      ...
      <RippleButton
        ...
        surfaceClassName={[
          ...
          items.length === 0
            ? 'bg-gray-800 opacity-50 text-cream cursor-not-allowed'
            : 'bg-maroon2 text-cream hover:bg-maroon2/90 active:bg-maroon2/80',
          'transition-colors',
        ].join(' ')}
      >
        Konfirmasi Pesanan
